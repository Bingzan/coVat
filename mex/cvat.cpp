...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) 
{
    ...
    if (mxGetNumberOfDimensions(prhs[0]) != 2 || !mxIsDouble(prhs[0])) {
        mexErrMsgIdAndTxt("incVat:cvat", "Incorrect input argument.");
    }
    ...
    if (rowNum != colNum) {
        mexErrMsgIdAndTxt("incVat:cvat", "Input distance matrix must be square.");
    }
        ...
        mxArray* mMst)
{
    ...
    mwIndex* pMMstIr = mxGetIr(mMst); // obsolete (64-bit mex files using sparse matrices must be rebuilt with the "-largeArrayDims" option.  See the R2006b release notes for more
                                      // details.)
...
//         cout << endl;
    ...
    C_Indices::iterator it = lIndices.begin();
...
//     std::cout << vMstDestIndices.size() << std::endl;
    C_PairIndices::iterator itPairs  = vMstDestIndices.begin();
...
void usageMessage() 
{
    mexErrMsgIdAndTxt("incVat:cvat","Usage: [mRearrangedDis, vPermVerts, vMstChain, vMinWeights, mMst, vVertPos] = cvat(mDis).");   
} // end of errorMessage()
